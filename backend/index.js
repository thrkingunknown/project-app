    ...
    try {
        ...
        try {
            ...
            if (dbState === 1) {
                ...
                var admin = db.admin();
                var serverStatus = await admin.serverStatus();

                healthStatus.services.mongodb.version = serverStatus.version;
                healthStatus.services.mongodb.uptime = serverStatus.uptime;
                healthStatus.services.mongodb.connections = serverStatus.connections;
            }
        } catch (dbError) {
            healthStatus.services.mongodb = {
                status: 'unhealthy',
        ...
        try {
            healthStatus.services.email = {
                status: transporter ? 'configured' : 'not configured',
                service: 'gmail'
            };
        } catch (emailError) {
            healthStatus.services.email = {
        ...
        var unhealthyServices = Object.values(healthStatus.services).filter(service =>
            service.status === 'unhealthy'
        );

        if (unhealthyServices.length > 0) {
            healthStatus.status = 'unhealthy';
        } else if (Object.values(healthStatus.services).some(service =>
            service.status === 'degraded'
        )) {
            healthStatus.status = 'degraded';
        }

        var httpStatus = healthStatus.status === 'healthy' ? 200 :
